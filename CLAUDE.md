...
Sections, in order:
1. **Beta hero** (`.beta-hero`) — dark bg, "You're in! Now let's get PADE on your phone." headline. Copy is deliberately framed as already-approved, not an application — visitors only reach this page after being personally invited, so nothing here should read as "request access."
2. **Step 01 — We need your info** (`.beta-section`, dark bg) — a `.beta-checklist` of two stacked label+description blocks (`.beta-checklist-item`, lime left-border accent), one for iPhone/Apple ID, one for Android/Gmail, each on its own block rather than run into a paragraph (short scannable chunks, not a wall of text — this was explicit user feedback, see "Beta page design notes" below). Below that, a signup form (`#beta-signup-form`) extending the `sheet-signup` pattern (see below) with extra fields, plus a `mailto:accounts@tfo.work` fallback for anyone who'd rather email it directly. CTA button reads "Send Info for Access" (not "Request Beta Access") — same reasoning as the hero copy.
3. **Step 02 — Install the beta** (`.beta-section-alt`, off-white bg) — two `.beta-platform-card` cards (iPhone/TestFlight, Android/Google Play closed testing) side-by-side on desktop (`min-width: 860px`), stacked on mobile. Each card is a numbered `.beta-steps` list (CSS counters, not `<ol>` default markers) walking through: install TestFlight / get the opt-in link → accept invite → install → open app → allow Camera + Notifications permissions → create account and set up first gig. A `.beta-note-box` below the grid sets expectations on TestFlight's 90-day build expiry and limited Google Play beta slots. The "02" step number is forced blue (`.beta-section-alt .beta-step-num`) on this section specifically — see contrast note below.
...
**Beta page design notes (from direct user feedback):**
- **Never render lime/green text on a grey or off-white background** — it's low contrast. `.beta-step-num` defaults to lime for dark sections, but is overridden to blue via `.beta-section-alt .beta-step-num` for the off-white Step 02 section. Check this any time a lime-colored element gets reused on a light-bg section elsewhere on the site.
- **Break instructional copy into short, scannable chunks (bullets/stacked blocks), not paragraphs** — this page is meant to be "REALLY simple and EASY to follow" per the user, since it's a functional onboarding flow, not marketing copy. Avoid flexbox-row-plus-`::before`-arrow bullet layouts — they can wrap awkwardly mid-sentence at narrow widths; prefer a stacked label (its own line) + description block instead.
- **CTA/button copy on this page must reflect that the visitor is already approved** — they only land here after being personally invited, so any language implying an application or request ("Request Beta Access") is wrong; frame it as adding them / collecting the info needed to add them.
